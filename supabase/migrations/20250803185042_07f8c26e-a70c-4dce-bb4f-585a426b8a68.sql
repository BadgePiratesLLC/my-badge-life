...
USING (EXISTS (
  ...
  WHERE b.id = badge_images.badge_id
    AND ((p.role = 'admin') OR 
         (p.role = 'maker' AND p.maker_approved = true AND p.assigned_team = b.team_name))
));
...
CREATE INDEX idx_badge_images_display_order ON public.badge_images(badge_id, display_order);
...
-- Trigger to ensure only one primary image per badge
CREATE OR REPLACE FUNCTION public.ensure_single_primary_badge_image()
RETURNS TRIGGER AS $$
BEGIN
  -- If setting this image as primary, unset all other primary images for this badge
  IF NEW.is_primary = true THEN
    UPDATE public.badge_images 
    SET is_primary = false 
    WHERE badge_id = NEW.badge_id AND id != NEW.id;
  END IF;
  
  -- If no primary image exists for this badge, make this one primary
  IF NOT EXISTS (
    SELECT 1 FROM public.badge_images 
    WHERE badge_id = NEW.badge_id AND is_primary = true AND id != NEW.id
  ) THEN
    NEW.is_primary = true;
  END IF;
...
CREATE TRIGGER ensure_single_primary_badge_image_trigger
BEFORE INSERT OR UPDATE ON public.badge_images
FOR EACH ROW
EXECUTE FUNCTION public.ensure_single_primary_badge_image();
...
-- Add helpful view for badges with their primary image
CREATE OR REPLACE VIEW public.badges_with_primary_image AS
SELECT 
  b.*,
  bi.image_url as primary_image_url,
  bi.id as primary_image_id
FROM public.badges b
LEFT JOIN public.badge_images bi ON b.id = bi.badge_id AND bi.is_primary = true;